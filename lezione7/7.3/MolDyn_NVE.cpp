...
//        ConfXYZ(nconf);//Write actual configuration in XYZ format //Commented to avoid "filesystem full"!
        nconf += 1;
        // printing the hist_block
        if(Num_mis==100){
          Print_hist_block(Num_mis,argv[2]);
          Clean_hist_block();
          Num_mis=1;
        }else{
          Num_mis++;
  ...
  Final_g_err(Num,argv[2]);
  cout << Num << endl;
  string type[5] = {"epot","ekin","temp","etot","pres"};
  Experiment Md;
...
void Measure(string s){ //Properties measurement
  ...
  Pres.open("data/output_pres_"+s+".dat",ios::app);
...
void Print_hist_block(int m, string s){

  ofstream Write;
  Write.open("data/histogram_blocks_"+s+".0",ios::app);
...
void Final_g_err(int nblk, string s){

    ifstream Read;
    double sum=0, sum2=0, err=0;
    Read.open("data/histogram_blocks_"+s+".0");
    ...
    cout << 0 << endl;
    ofstream Write("data/final_hist_"+s+".0");
